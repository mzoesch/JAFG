...
MODIFICATIONUBTEDITORTOOL_API auto LexToString(const EModificationUBTEditorToolStartGame::Type InType) -> const TCHAR*;
...
struct MODIFICATIONUBTEDITORTOOL_API FModificationUBTEditorToolTargetSettings
{
    ...
    bool bCopyPluginsToGame = false;

    UPROPERTY(EditAnywhere, Config, Category = Config, DisplayName = "Copy to Compiled Game Path", meta = (EditCondition = bCopyPluginsToGame))
...
public:
    ...
    EBuildConfiguration GetBuildConfiguration(void) const;
    const TMap<FString, FModificationUBTEditorToolTargetSettings> GetPlatformTargetSettings(void) const;
